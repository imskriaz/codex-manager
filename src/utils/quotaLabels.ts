...
import type { CodexAdditionalQuotaLimit } from "../core/types";
...
/** Provider reserve capacity is internal metadata, not a quota the user can act on. */
export function isUserFacingAdditionalQuotaLimit(
  limit: Pick<CodexAdditionalQuotaLimit, "limitName" | "meteredFeature">
): boolean {
  return ![limit.limitName, limit.meteredFeature].some((value) =>
    value
      ?.trim()
      .toLowerCase()
      .split(/[^a-z0-9]+/)
