 ...
 * 账号记录
 */
export interface CodexManagerAccountRecord {
  /** 内部存储 ID */
  id: string;
  /** 登录时间戳 (毫秒) */
  loginAt?: number;
...
export interface CodexManagerRestoreResult {
...
export interface SharedCodexManagerAccountJson {
  id?: string;
  email?: string;
  auth_mode?: string;
  OPENAI_API_KEY?: string | null;
...
export interface CodexManagerBackup {
  format: "codex-manager-backup";
  version: 1;
  exportedAt: string;
  accounts: SharedCodexManagerAccountJson[];
  activeAccountId?: string;
  /** Configuration values under the codexManager section. */
...
export interface CodexAutoSwitchReason {
  ...
  toAccountId: string;
  toEmail: string;
  trigger: "hourly" | "weekly" | "hourly_and_weekly" | "reset";
  matchedRules: string[];
