...
import {
  ...
  isUserFacingAdditionalQuotaLimit,
...
export async function buildDashboardState(
  ...
  const [resolvedCodexAppPath, indexHealth, accounts] = await Promise.all([
    resolveCodexAppLaunchPath(baseSettings.codexAppPath),
    repo.getIndexHealthSummary(),
    repo.listAccounts()
  ]);
  const settings = {
    ...baseSettings,
    encryptedSyncRegistryOverrideEnabled: isEncryptedSyncRegistryOverrideEnabled(),
    resolvedCodexAppPath: resolvedCodexAppPath ?? ""
  };
...
export function resolveTerminalNotice(
  ...
  activeAccount: CodexManagerAccountRecord | undefined,
  copy: DashboardCopy
  ...
  const message = template.replace("{account}", activeAccount.email);
  return {
    level: notice.level,
    // A window reload can interrupt an in-flight Codex turn. Keep the normal
    // switch wording, but make the recovery path explicit in the browser dashboard
    // dashboards where the VS Code toast may no longer be visible.
    message:
      notice.switchResult === "switched-and-reloaded"
        ? `${message} If a Codex chat was interrupted, resume it from Sessions.`
        : message,
...
export function resolveDashboardQueuedSwitch(
  accounts: readonly Pick<CodexManagerAccountRecord, "id">[],
...
function mapAccount(
  ...
  if ((resetCreditsAvailable ?? 0) > 0 && resetCreditsNextExpiresAt == null) {
    console.info("[codexManager] dashboard state missing reset credits expiry", {
  ...
  return {
    ...
    runningDeviceOnline: accountLease?.online,
...
export function buildMetrics(
  ...
  for (const [index, limit] of quota?.additionalRateLimits?.entries() ?? []) {
    if (!isUserFacingAdditionalQuotaLimit(limit)) {
      continue;
    }
...
function hasQuota402(account: CodexManagerAccountRecord): boolean {
...
function resolveWorkspaceDisplay(account: CodexManagerAccountRecord): string {
...
function isTeamWorkspace(account: CodexManagerAccountRecord): boolean {
...
function formatPlanTypeWithQuota(account: CodexManagerAccountRecord, lang: DashboardState["lang"]): string {
...
function inferProQuotaMultiplier(account: CodexManagerAccountRecord): "5x" | "20x" | undefined {
