...
export type SyncAccountEntry = SharedCodexManagerAccountJson;
...
export class EncryptedSyncManager implements vscode.Disposable {
  ...
  private currentSyncAnnouncesSuccess = false;
  ...
  setOnlineDeviceIds(deviceIds: readonly string[] | undefined): void {
    const next = deviceIds ? new Set(deviceIds.filter((deviceId) => deviceId.trim().length > 0)) : undefined;
    const before = visibleOnlineDeviceIds ? [...visibleOnlineDeviceIds].sort().join("\n") : "unknown";
    const after = next ? [...next].sort().join("\n") : "unknown";
    visibleOnlineDeviceIds = next;
    if (before !== after) this.onStateChanged?.();
  }

  getRealtimeEnablementRegistry(): SyncAccountEnablement[] {
    return this.readLocalEnablement();
  }

  getPresenceDeviceId(): Promise<string> {
  ...
  async signRealtimePeerPayload(payload: string): Promise<string | undefined> {
    ...
    return crypto
      .createHmac("sha256", crypto.createHash("sha256").update(`peer-presence-v1:${passphrase}`, "utf8").digest())
      .update(payload, "utf8")
      .digest("base64url");
  }

  /**
   * Returns the opaque HMAC key used by the optional detached relay. The
   * relay never receives the sync passphrase itself; callers should persist
   * this value with user-only file permissions.
   */
  async getRealtimeRelayKey(): Promise<string | undefined> {
    if (!this.isEnabled()) return undefined;
    const passphrase = await this.context.secrets.get(PASSPHRASE_KEY);
    if (!passphrase) return undefined;
  ...
  /** Build one encrypted vault snapshot for the authenticated peer channel. */
  async getRealtimeEncryptedVault(): Promise<string | undefined> {
    if (!this.isEnabled()) return undefined;
    if (this.realtimeVaultCache?.mutationVersion === this.mutationVersion) {
      return this.realtimeVaultCache.encrypted;
    }
    const passphrase = await this.context.secrets.get(PASSPHRASE_KEY);
    if (!passphrase) return undefined;
    ...
    this.realtimeVaultCache = { mutationVersion: this.mutationVersion, encrypted };
    return encrypted;
  }
  ...
  async applyRealtimeEncryptedVault(raw: string): Promise<boolean> {
    ...
    try {
      ...
      if (!isVaultAuthenticationError(error)) {
        console.warn("[codexManager] real-time encrypted vault rejected:", error);
      }
      return false;
    }
  }
  ...
  async verifyRealtimePeerPayload(payload: string, signature: string): Promise<boolean> {
    const expected = await this.signRealtimePeerPayload(payload);
  ...
  /** Merge peer state immediately; VS Code Settings Sync remains the durable delayed transport. */
  async applyRealtimeEnablementRegistry(entries: readonly SyncAccountEnablement[]): Promise<void> {
    if (!this.isEnabled()) return;
    const merged = mergeSyncAccountEnablement(this.readLocalEnablement(), entries);
    if (syncEnablementFingerprint(merged) === syncEnablementFingerprint(this.readLocalEnablement())) return;
  ...
  /**
   * Heartbeat loss must not rewrite a user's explicit local enablement choice.
   * Eligibility and ownership checks gate automated work separately.
   */
  async fenceLocalAccountsAfterPresenceLoss(): Promise<string[]> {
  ...
  async prepareAccountEnablement(accountId: string, enabled: boolean): Promise<void> {
    ...
    await this.mutationChain.catch(() => undefined);
    await this.refreshEnablementFromDownloadedVault();
    const current = this.findForeignEnablement(accountId);
    if (enabled && current) {
      throw new Error(
        `This account is enabled on ${current.deviceName}. Wait for it to go offline or disable it there first.`
      );
    }
  }
  ...
  async setRegistryOverrideEnabled(enabled: boolean, options?: { passphrase?: string }): Promise<boolean> {
    ...
    if (enabled) {
      ...
      // Rescue permits local enablement to diverge from synchronized ownership.
      // Before turning it off, refresh and reapply the shared registry so the
      // dashboard cannot report that enforcement is restored while overrides
      // are still active locally.
      await this.mutationChain.catch(() => undefined);
      await this.refreshEnablementFromDownloadedVault();
      const sharedEnablement = this.readLocalEnablement();
      if (sharedEnablement.length > 0) {
        await this.applyMergedEnablement(sharedEnablement, true);
      }
    }
  ...
  async syncNow(interactive = true, announceSuccess = interactive, syncSettings = interactive): Promise<boolean> {
    ...
    try {
      ...
      if (this.currentSyncTask === undefined) {
        this.currentSyncAnnouncesSuccess = false;
        this.currentSyncUsesSettingsSync = false;
      }
    }
  }
  ...
  private async performSyncNow(
    ...
    try {
      ...
      if (!rawRemote && local.accounts.length === 0) {
        ...
        // This is inconclusive, not a successful sync. Keep durable work
        // queued so the next Settings Sync download can be merged.
        return false;
      }
      ...
      // Settings Sync can download a newer vault while this merge is doing
      // crypto or repository work. Never apply or publish a candidate built
      // from an obsolete remote snapshot; retry from the latest ciphertext.
      if (this.context.globalState.get<string>(SYNC_KEY) !== rawRemote) {
        return this.retryAfterVaultConflict(interactive, announceSuccess, syncSettings, conflictAttempt);
      }
      ...
      const expectedVaultRaw = wroteSyncVault ? this.lastRemoteRaw : rawRemote;
      if (this.context.globalState.get<string>(SYNC_KEY) !== expectedVaultRaw) {
        return this.retryAfterVaultConflict(interactive, announceSuccess, syncSettings, conflictAttempt);
      }
      ...
      console.error("[codexManager] encrypted sync failed:", error);
      if (isVaultAuthenticationError(error)) {
        encryptedSyncNeedsConfiguration = true;
        ...
        try {
          ...
          console.warn("[codexManager] could not clear the cached encrypted-sync passphrase:", clearError);
        }
        this.lastRemoteRaw = undefined;
        this.lastRemotePayload = undefined;
        this.lastRemotePassphraseHash = undefined;
  ...
  private async retryAfterVaultConflict(
    interactive: boolean,
    announceSuccess: boolean,
    syncSettings: boolean,
...
export function createSyncEntry(entry: SharedCodexManagerAccountJson): SyncAccountEntry {
  return {
    id: entry.id,
    email: entry.email,
...
export async function encryptSyncPayload(payload: SyncPayload, passphrase: string): Promise<string> {
  ...
  const header = {
    format: "codex-manager-encrypted-sync" as const,
    version: 1 as const,
...
function validateSyncPayload(value: unknown): asserts value is SyncPayload {
  ...
  if (
    payload.format !== "codex-manager-encrypted-sync" ||
    payload.version !== 1 ||
...
function parseCipherEnvelope(raw: string): ParsedCipherEnvelope {
  ...
  if (
    envelope.format !== "codex-manager-encrypted-sync" ||
    envelope.version !== 1 ||
