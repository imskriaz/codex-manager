...
export function buildDashboardStateSignature(state: DashboardState): string {
  const accountSignature = state.accounts
    .map((account) =>
      [
        ...
        account.runningDeviceOnline === undefined ? "?" : account.runningDeviceOnline ? "1" : "0",
        account.enabled ? "1" : "0",
  ...
  return [
    ...
    state.settings.webDashboardAlwaysOnlineEnabled ? "1" : "0",
