    ...
    if (!primary) {
      ...
      md.appendMarkdown(
        "\n\n**Quick access**  \n" +
          ...
          "Command: **Codex Manager: Keyboard Shortcuts & Help**"
      );
...
export function buildStatusText(account: CodexManagerAccountRecord, showHourlyQuota: boolean): string {
  ...
  const hourly = quota?.hourlyWindowPresent ? quota.hourlyPercentage : undefined;
  const weekly = quota?.weeklyWindowPresent ? quota.weeklyPercentage : undefined;
...
export function buildStatusQuickAccessMarkdown(): string {
  return (
    "**Quick access**  \n" +
    "`Ctrl+Shift+P` — find every **Codex Manager** command  \n" +
    "`Ctrl+K Ctrl+S` — view or assign keyboard shortcuts  \n" +
...
function formatWorkspace(account: CodexManagerAccountRecord): string {
...
function formatCredits(account: CodexManagerAccountRecord): string | undefined {
