...
export async function readAideckCodexTokens(accountId: string): Promise<AideckMirrorTokenSnapshot | undefined> {
  const filePath = getAideckCodexManagerAccountFilePath(accountId);
  try {
...
export async function listAideckCodexSharedAccounts(): Promise<SharedCodexManagerAccountJson[]> {
  ...
  try {
    ...
    for (const item of accounts) {
      ...
      if (id) {
        accountFiles.add(getAideckCodexManagerAccountFilePath(id));
      }
    }
  } catch {}
  ...
  const shared: SharedCodexManagerAccountJson[] = [];
  for (const filePath of accountFiles) {
    const parsed = await readJsonFile(filePath);
    const entry = parsed ? toSharedCodexManagerAccount(parsed) : undefined;
    if (entry) {
      shared.push(entry);
    }
  }
...
export async function mirrorAideckCodexManagerAccount(account: CodexManagerAccountRecord, tokens?: CodexTokens): Promise<void> {
  ...
  try {
    const now = Date.now();
    const filePath = getAideckCodexManagerAccountFilePath(account.id);
...
export async function removeAideckCodexManagerAccount(accountId: string): Promise<void> {
  if (!accountId.trim()) {
    return;
  }

  try {
    await fs.rm(getAideckCodexManagerAccountFilePath(accountId), { force: true });
...
export function getAideckCodexManagerAccountFilePath(accountId: string): string {
...
function toSharedCodexManagerAccount(account: JsonRecord): SharedCodexManagerAccountJson | undefined {
...
function getSharedQuota(value: unknown): SharedCodexManagerAccountJson["quota"] {
  const quota = getRecord(value);
  return quota ? (quota as NonNullable<SharedCodexManagerAccountJson["quota"]>) : null;
}

function getSharedQuotaError(value: unknown): SharedCodexManagerAccountJson["quota_error"] {
...
function shouldMirrorTokensForAccount(account: CodexManagerAccountRecord, tokens: CodexTokens | undefined): tokens is CodexTokens {
