...
export class AccountsRepository {
  ...
  private invalidateTokenCacheForAccount(accountId: string): void {
  ...
  scheduleStartupSync(onComplete: () => void, delayMs = 5_000): void {
    ...
    this.startupSyncTimer = setTimeout(() => {
      ...
      void runCentralAccountOperationWithCooldown("Deferred startup account sync", 5 * 60 * 1000, async () => {
        ...
        .catch((error: unknown) => {
          console.warn("[codexManager] deferred startup account sync skipped:", error);
   ...
   * 获取所有账号列表
   */
  async listAccounts(): Promise<CodexManagerAccountRecord[]> {
    try {
   ...
   * 获取单个账号
   */
  async getAccount(accountId: string): Promise<CodexManagerAccountRecord | undefined> {
    try {
  ...
  async restoreIndexFromLatestBackup(): Promise<CodexManagerRestoreResult> {
  ...
  async restoreAccountsFromAuthFile(): Promise<CodexManagerRestoreResult> {
  ...
  ): Promise<CodexManagerRestoreResult> {
    let restored: CodexManagerAccountRecord[];
    try {
  ...
  async updateTokens(accountId: string, tokens: CodexTokens): Promise<CodexManagerAccountRecord> {
    const index = await this.readIndex();
    const account = index.accounts.find((item) => item.id === accountId);
  ...
  async refreshAccountProfileMetadata(accountId: string): Promise<CodexManagerAccountRecord> {
    const index = await this.readIndex();
    const account = index.accounts.find((item) => item.id === accountId);
    if (!account) {
      throw createError.accountNotFound(accountId);
  ...
  async dismissHealthIssue(accountId: string, issueKey?: string): Promise<CodexManagerAccountRecord> {
    const index = await this.readIndex();
  ...
  private async upsertFromTokensInternal(
    ...
    options: {
      ...
      restoreSource?: CodexManagerRestoreResult["source"];
      addedVia?: CodexManagerAccountRecord["addedVia"];
      identityFallback?: {
        email?: string;
        accountId?: string;
   ...
   * 导入当前 auth.json
   */
  async importCurrentAuth(): Promise<CodexManagerAccountRecord> {
    const auth = await readAuthFile();
    if (!auth) {
      throw new AccountError("Current auth.json was not found", {
        code: ErrorCode.NOT_FOUND,
  ...
  /** Inspect the active auth.json without mutating the saved account index. */
  async inspectCurrentAuth(): Promise<{ email?: string; alreadyAdded: boolean }> {
    const auth = await readAuthFile();
    if (!auth?.tokens?.id_token || !auth.tokens.access_token) {
    ...
    try {
      ...
      if (!email) return { alreadyAdded: false };
      const id = buildAccountStorageId(email, claims.accountId ?? auth.tokens.account_id, claims.organizationId);
  ...
  async exportSharedAccounts(accountIds: string[]): Promise<SharedCodexManagerAccountJson[]> {
    ...
    const sharedAccounts: SharedCodexManagerAccountJson[] = [];
  ...
  private async importSharedAccountsInternal(
    ...
    const entries = toSharedEntries(input);
    const imported: CodexManagerAccountRecord[] = [];
  ...
  private async switchAccountLocked(accountId: string, forceTokenRefresh = false): Promise<CodexManagerAccountRecord> {
    const index = await this.readIndex();
    const account = index.accounts.find((item) => item.id === accountId);
    ...
    if (forceTokenRefresh && tokens.refreshToken) {
      ...
      backgroundRotation = async () => {
        try {
          ...
          console.warn(`[codexManager] token rotation failed after switching ${account.email}:`, error);
        }
      };
    }
   ...
   * @returns 更新后的账号记录
   */
  async setStatusBarVisibility(accountId: string, visible: boolean): Promise<CodexManagerAccountRecord> {
    const index = await this.readIndex();
  ...
  async setAccountEnabled(accountId: string, enabled: boolean): Promise<CodexManagerAccountRecord> {
  ...
  private async readIndexInternal(): Promise<CodexManagerIndex> {
  ...
  private async readIndexForRecovery(): Promise<CodexManagerIndex> {
  ...
  private async persistIndex(index: CodexManagerIndex): Promise<void> {
  ...
  private async persistRecoveredIndex(
    ...
    source: CodexManagerRestoreResult["source"]
  ): Promise<void> {
