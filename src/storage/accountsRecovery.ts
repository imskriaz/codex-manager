...
export async function restoreMissingIndex(
  ...
  console.info("[codexManager] accounts index not found, using empty index");
  state.indexHealth = {
    status: "healthy",
...
export async function tryRestoreFromBackups(params: {
  ...
  for (let slot = 1; slot <= params.backupCount; slot += 1) {
    ...
    try {
      ...
      console.warn(`[codexManager] restored accounts index from backup-${slot}`);
      ...
      if (!isFileNotFoundError(error)) {
        console.error(`[codexManager] failed to restore accounts index from backup-${slot}:`, error);
      }
    }
  }
