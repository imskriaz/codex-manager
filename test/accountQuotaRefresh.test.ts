...
vi.mock("../src/application/accounts/switchEffects", () => ({
  ...
  reloadWindowNow: reloadWindowNowMock
}));
...
describe("refreshSingleQuota token automation state", () => {
  ...
  it("fetches reset credits expiry from the updated quota snapshot", async () => {
    const updatedAccount: CodexManagerAccountRecord = {
      ...account,
      accountId: "acct-1",
      quotaSummary: {
        hourlyPercentage: 82,
  ...
  it("auto-switches to the candidate with the highest 5-hour quota before weekly quota", async () => {
    ...
    const active: CodexManagerAccountRecord = {
      id: "active",
      email: "dev@example.com",
      isActive: true,
      createdAt: 1,
  ...
  it("allows a user-triggered Auto Select to use automation-disabled accounts", async () => {
    ...
    const active = { ...createAccount("active-disabled", true, 90, 5), enabled: false };
    const candidate = { ...createAccount("candidate-disabled", false, 90, 80), enabled: false };
    const repo = {
  ...
  it("shows a warning when the threshold is crossed but no safe candidate exists", async () => {
    ...
    expect(getAutoSwitchRuntimeSnapshot().dashboardNotice).toMatchObject({
      level: "warning",
      message: "No account to switch — no capable account has enough quota remaining."
    });
  });
  ...
  it("refreshes non-current quotas before a warning selects a switch target", async () => {
    ...
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          ...
          autoSwitchHourlyThreshold: 20,
          autoSwitchWeeklyThreshold: 20,
          autoSwitchRefreshAllBeforeSwitchEnabled: true,
    ...
    refreshQuotaMock.mockImplementation(async (account: CodexManagerAccountRecord) => ({
      quota: account.quotaSummary,
      error: undefined
    }));
  ...
  it("runs the reset plan when every enabled account is out of quota", async () => {
    vi.mocked(vscode.window.showInformationMessage).mockClear();
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          ...
          autoResetWeeklyThreshold: 10
        };
        return values[key] ?? defaultValue;
      }),
      update: vi.fn()
    } as never);
    const active = createAccount("reset-active", true, 0, 0);
    const resetTarget = createAccount("reset-target", false, 0, 0);
  ...
  it("resets the active account in place without switching", async () => {
    vi.mocked(vscode.window.showInformationMessage).mockClear();
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
  ...
  it("does not switch to a below-threshold fallback when an active-account reset fails", async () => {
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          autoSwitchEnabled: true,
    ...
    const active = createAccount("failed-active-reset", true, 0, 0);
    active.quotaSummary!.resetCreditsAvailable = 1;
    ...
    expect(repo.switchAccount).not.toHaveBeenCalled();
    expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(expect.stringContaining("Auto switch failed"));
  });

  it("does not switch past a reset candidate to a below-threshold account", async () => {
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          autoSwitchEnabled: true,
    ...
    const active = createAccount("failed-target-active", true, 0, 0);
    const resetTarget = createAccount("failed-reset-target", false, 0, 0);
    resetTarget.quotaSummary!.resetCreditsAvailable = 1;
...
describe("quota warning window validation", () => {
  ...
  it("opens the account picker when Select Account is chosen", async () => {
    ...
    await maybeWarnForAccount(repo as unknown as AccountsRepository, account.id);
    await vi.waitFor(() => expect(executeCommand).toHaveBeenCalledWith("codexManager.switchAccount"));
  });
  ...
  it("uses a separate weekly warning threshold", async () => {
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          quotaWarningEnabled: true,
      ...
      inspect: vi.fn((key: string) =>
        key === "quotaWarningWeeklyThreshold"
          ? { key: "codexManager.quotaWarningWeeklyThreshold", globalValue: 30, defaultValue: 20 }
          : undefined
      )
    } as never);
    const showWarning = vi.spyOn(vscode.window, "showWarningMessage").mockResolvedValue(undefined);
    showWarning.mockClear();
    const account = createAccount("separate-weekly-warning", true, 15, 25);
    const target = createAccount("separate-weekly-target", false, 90, 85);
    const repo = {
      getAccount: vi.fn(async () => account),
      listAccounts: vi.fn(async () => [account, target])
    };
    ...
    expect(showWarning).toHaveBeenCalledTimes(1);
    expect(showWarning.mock.calls[0]?.[0]).toContain(
      "separate-weekly-warning@example.com Weekly quota is at 25%, below your configured threshold of 30%."
    );
    expect(showWarning.mock.calls[0]?.[0]).not.toContain("5h quota");
  });

  it("does not warn from a quota snapshot captured before the account was switched in", async () => {
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          quotaWarningEnabled: true,
    ...
    const showWarning = vi.spyOn(vscode.window, "showWarningMessage").mockResolvedValue(undefined);
    showWarning.mockClear();
    const account = createAccount("stale-after-switch", true, 80, 5);
    account.lastQuotaAt = 100;
    account.sessionStartedAt = 200;
    const repo = {
      getAccount: vi.fn(async () => account),
      listAccounts: vi.fn(async () => [account])
    };
  ...
  it("does not auto-switch from a quota snapshot captured before the account was switched in", async () => {
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          autoSwitchEnabled: true,
    ...
    const account = createAccount("stale-auto-switch", true, 80, 5);
    account.lastQuotaAt = 100;
    account.sessionStartedAt = 200;
    const target = createAccount("stale-auto-target", false, 90, 90);
    const repo = {
      listAccounts: vi.fn(async () => [account, target]),
      switchAccount: vi.fn()
    };
  ...
  it("cancels auto-switch when another switch changes the active account during evaluation", async () => {
    vi.spyOn(vscode.workspace, "getConfiguration").mockReturnValue({
      get: vi.fn((key: string, defaultValue?: unknown) => {
        const values: Record<string, unknown> = {
          autoSwitchEnabled: true,
    ...
    const active = createAccount("auto-race-active", true, 80, 5);
  ...
  it("cancels a warning when the account becomes inactive before notification display", async () => {
    ...
    const active = createAccount("warning-race", true, 80, 5);
    ...
    await maybeWarnForAccount(repo as unknown as AccountsRepository, active.id);

    expect(showWarning).not.toHaveBeenCalled();
  });
});

function createAccount(id: string, isActive: boolean, hourly: number, weekly: number): CodexManagerAccountRecord {
  return {
    id,
