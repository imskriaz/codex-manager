...
describe("buildStatusText", () => {
  ...
  it("shows only the available quota instead of a normalized placeholder", () => {
    const withoutHourlyQuota = {
      ...account,
      quotaSummary: {
    ...
    expect(buildStatusText(withoutHourlyQuota, true)).toBe("$(codex-openai) dev@example.com 82%");
  });
});

describe("status bar tooltip", () => {
  it("does not repeat the instruction to open the quota panel", () => {
    const quickAccess = buildStatusQuickAccessMarkdown();

    expect(quickAccess).toContain("Quick access");
    expect(quickAccess).not.toContain("Click this status item");
  });
});

describe("status bar activation states", () => {
  it("keeps the quota webview command clickable while starting or recovering", () => {
    const item = {
    ...
    const windowApi = vscode.window as unknown as {
      createStatusBarItem: () => typeof item;
    };
    const previousFactory = windowApi.createStatusBarItem;
    windowApi.createStatusBarItem = () => item;
    try {
      const context = { subscriptions: [] as Array<{ dispose(): void }> } as unknown as vscode.ExtensionContext;
      const provider = new AccountsStatusBarProvider(context, { listAccounts: async () => [] } as never);

      provider.showStarting();
      expect(item.command).toBe("codexManager.showQuotaSummary");
      provider.showActivationFailure("test failure");
      expect(item.command).toBe("codexManager.showQuotaSummary");
    } finally {
      windowApi.createStatusBarItem = previousFactory;
    }
  });
});
...
describe("renderAccountPanel", () => {
  ...
  it("does not render provider reserve quota", () => {
    ...
    expect(panel).not.toContain("gpt-reserve");
