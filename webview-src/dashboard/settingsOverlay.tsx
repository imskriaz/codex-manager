      ...
      >
        <div class="settings-modal-head">
          <div class="settings-modal-heading">
            ...
            <div class="settings-modal-title" id="settings-modal-title">{props.copy.settingsTitle}</div>
          </div>
          ...
          >
            ...
            <div class="settings-block settings-block-wide settings-integration-group">
              <div class="settings-block-head settings-integration-head">
                <div class="settings-block-title">{props.lang === "zh" ? "面板" : props.lang === "zh-hant" ? "面板" : "Dashboard"}</div>
                <div class="settings-block-sub">{props.lang === "zh" ? "配置浏览器访问、在线主机和公共域名。" : props.lang === "zh-hant" ? "設定瀏覽器存取、線上主機與公開網域。" : "Configure browser access, the online host, and public domain."}</div>
              </div>
            ...
            <SettingsToggleBlock
              ...
              sub={
                <>
                  {props.lang === "zh"
                    ? "在 127.0.0.1:39875 启动浏览器面板。本机访问无需密码；通过 Cloudflared 暴露前请设置密码。"
                    : props.lang === "zh-hant"
                      ? "在 127.0.0.1:39875 啟動瀏覽器面板。本機存取無需密碼；透過 Cloudflared 公開前請設定密碼。"
                      : "Run the browser dashboard at 127.0.0.1:39875. Local access does not require a password; set one before exposing it through Cloudflared. "}
            ...
            <SettingsToggleBlock
              title={props.lang === "zh" ? "始终在线 WebSocket 主机" : props.lang === "zh-hant" ? "始終在線 WebSocket 主機" : "Always-online WebSocket host"}
              sub={props.lang === "zh" ? "在此电脑启动独立 Node.js 中继，即使 VS Code 关闭也保持多电脑 WebSocket 在线。建议只在一台常开电脑启用。" : props.lang === "zh-hant" ? "在此電腦啟動獨立 Node.js 中繼，即使 VS Code 關閉也保持多電腦 WebSocket 在線。建議只在一台常開電腦啟用。" : "Run a detached Node.js relay on this PC so multi-PC WebSockets stay available after VS Code closes. Enable it on one always-on PC."}
            ...
            >
              <div class="settings-note">
                {props.settings.webDashboardAlwaysOnlineEnabled
                  ? props.lang === "zh" ? "已启用。VS Code 关闭后中继会接管 127.0.0.1:39875；Cloudflared 端口无需改变。" : props.lang === "zh-hant" ? "已啟用。VS Code 關閉後中繼會接管 127.0.0.1:39875；Cloudflared 連接埠無需改變。" : "Enabled. The relay takes over 127.0.0.1:39875 after VS Code closes; Cloudflared needs no port change."
                  : props.lang === "zh" ? "已停用。VS Code 关闭后不会运行独立中继。" : props.lang === "zh-hant" ? "已停用。VS Code 關閉後不會執行獨立中繼。" : "Disabled. No detached relay runs after VS Code closes."}
              </div>
            </SettingsToggleBlock>
            <div class="settings-block settings-block-wide">
              <div class="settings-block-head">
                <div class="settings-block-title">{props.lang === "zh" ? "Cloudflared 域名" : props.lang === "zh-hant" ? "Cloudflared 網域" : "Cloudflared domain"}</div>
              ...
              <div class="settings-note">
                {props.lang === "zh"
                  ...
                  : props.lang === "zh-hant"
                    ...
                    : "Quick setup: run cloudflared tunnel --url http://127.0.0.1:39875, set a dashboard password, then share this HTTPS domain with other PCs."}
              </div>
              </div>
            </div>
            </div>
            <div class="settings-block settings-block-wide settings-integration-group">
              <div class="settings-block-head settings-integration-head">
                <div class="settings-block-title">{props.lang === "zh" ? "工作区" : props.lang === "zh-hant" ? "工作區" : "Workspace"}</div>
                <div class="settings-block-sub">{props.lang === "zh" ? "控制工作区功能；CLI 路径仍可单独配置。" : props.lang === "zh-hant" ? "控制工作區功能；CLI 路徑仍可單獨設定。" : "Control workspace features; configure the CLI path separately when needed."}</div>
              </div>
              <div class="settings-integration-grid">
            <SettingsToggleBlock
              title={props.lang === "zh" ? "启用工作区（实验性）" : props.lang === "zh-hant" ? "啟用工作區（實驗性）" : "Enable workspace (Experimental)"}
              sub={props.lang === "zh" ? "默认停用。启用后可在面板中打开工作区、会话和终端工具。" : props.lang === "zh-hant" ? "預設停用。啟用後可在面板中開啟工作區、工作階段與終端工具。" : "Disabled by default. Enable the workspace view, sessions, and terminal tools in the dashboard."}
            ...
            >
              <div class="settings-note">
                {props.settings.cliIntegrationEnabled
                  ? props.lang === "zh" ? "已启用。面板顶部会显示工作区入口。" : props.lang === "zh-hant" ? "已啟用。面板頂部會顯示工作區入口。" : "Enabled. The workspace entry is available at the top of the dashboard."
                  : props.lang === "zh" ? "已停用。工作区入口和会话读取均关闭。" : props.lang === "zh-hant" ? "已停用。工作區入口與工作階段讀取均關閉。" : "Disabled. The workspace entry and session access are off."
                }
              </div>
            </SettingsToggleBlock>
            <SettingsPathBlock
              ...
              compact
              title={props.lang === "zh" ? "Codex CLI 路径" : props.lang === "zh-hant" ? "Codex CLI 路徑" : "Codex CLI path"}
              sub={props.lang === "zh" ? "可选。指定 Codex CLI 可执行文件或启动脚本；留空则自动检测。" : props.lang === "zh-hant" ? "可選。指定 Codex CLI 可執行檔或啟動腳本；留空則自動偵測。" : "Optional. Set the Codex CLI executable or launcher script; leave empty for auto-detection."}
              emptyLabel={props.lang === "zh" ? "未设置 CLI 路径，将使用自动检测。" : props.lang === "zh-hant" ? "未設定 CLI 路徑，將使用自動偵測。" : "No custom CLI path. Automatic detection will be used."}
              pickLabel={props.lang === "zh" ? "选择 CLI 路径" : props.lang === "zh-hant" ? "選擇 CLI 路徑" : "Choose CLI path"}
              clearLabel={props.lang === "zh" ? "自动检测 CLI" : props.lang === "zh-hant" ? "自動偵測 CLI" : "Auto-detect CLI"}
            ...
            >
              <div class={`settings-stack ${props.settings.autoSwitchEnabled ? "" : "is-hidden"}`}>
                ...
                <SettingsToggleBlock
                  ...
                  sub={props.copy.autoResetSub ?? "Use an available reset credit when every enabled account is out of quota."}
                ...
                >
                  <div class={`settings-stack ${props.settings.autoResetEnabled ? "" : "is-hidden"}`}>
                    <SettingsDiscreteSlider
                      ...
                      description={(value) =>
                        formatTemplate(
                          props.copy.autoResetThresholdDescTemplate ?? "Reset a candidate when its weekly quota is {value}% or lower.",
                          value
                        )
                      }
...
function resolveSettingsNavigationCopy(lang: DashboardState["lang"]): {
  ...
  if (lang === "zh") {
    return {
      ...
      tabs: [
        ...
        { id: "automation", label: "自动化", heading: "配额与自动化", description: "管理刷新、告警、阈值和自动切换行为。" },
  ...
  if (lang === "zh-hant") {
    return {
      ...
      tabListLabel: "設定類別",
      tabs: [
        { id: "general", label: "一般", heading: "工作區與外觀", description: "設定主題、語言、面板存取和本機工作階段。" },
        { id: "automation", label: "自動化", heading: "配額與自動化", description: "管理重新整理、警告、門檻和自動切換行為。" },
  ...
  return {
    ...
    tabs: [
      { id: "general", label: "General", heading: "Workspace & appearance", description: "Theme, language, dashboard access, and local sessions." },
      { id: "automation", label: "Automation", heading: "Quota & automation", description: "Refresh schedules, alerts, thresholds, and account switching." },
      { id: "data", label: "Data & sync", heading: "Data, sync & diagnostics", description: "Backups, encrypted sync, recovery controls, and network logs." }
    ]
  };
}
