...
export function SettingsPathBlock(props: {
  ...
  const content = (
    <>
      <div class="settings-block-head">
        <div class="settings-block-title">{props.title ?? props.copy.appPathTitle}</div>
        <div class="settings-block-sub">{props.sub ?? props.copy.appPathSub}</div>
      </div>
      <div class="settings-note settings-path-note">{props.pathValue || props.emptyLabel || props.copy.appPathEmpty}</div>
      <div class="saved-actions settings-inline-actions">
        <button type="button" onClick={props.onPick}>
          {props.pickLabel ?? props.copy.pickPath}
        </button>
        <button type="button" disabled={!props.hasCustomPath} onClick={props.onClear}>
          {props.clearLabel ?? props.copy.clearPath}
        </button>
      </div>
    </>
  );
      ...
      <div class="threshold-dual">
        ...
        <div class="threshold-range-stack">
          ...
          <input
            ...
            type="range"
            aria-label={`${props.copy.colorThresholdYellowTitle} threshold`}
            min="0"
            max="100"
            step="1"
            value={yellow}
          ...
          <input
            ...
            type="range"
            aria-label={`${props.copy.colorThresholdGreenTitle} threshold`}
            min="0"
            max="100"
            step="1"
            value={green}
    ...
    <div class={`step-slider step-slider-${props.accent}`}>
      ...
      <div class="step-slider-stack">
        ...
        <input
          ...
          type="range"
          aria-label={props.description(currentValue)}
          min={String(minValue)}
          max={String(maxValue)}
          step="1"
          value={currentValue}
