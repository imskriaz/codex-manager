...
import type { DashboardAccountViewModel, DashboardActionName, DashboardState } from "../../src/domain/dashboard/types";
...
export function BrowserActionModal(props: {
  ...
  onConfirm: (request: BrowserActionRequest, submittedValues?: string[]) => void;
            ...
            <div class="browser-account-picker" role="listbox" aria-label={title}>
              {candidates.map((account) => (
                ...
                >
                  ...
                  <small>
                    {account.planTypeLabel} · {account.workspaceLabel}
                  </small>
                </button>
              ))}
            </div>
        ...
        >
          ...
          <div class="modal-actions">
            ...
            <button
              ...
              type="submit"
              disabled={!password || mismatch || (request.requireCurrentPassword && !currentPassword)}
            >
              {submitLabel}
            </button>
          </div>
  ...
  if (request.kind === "notification") {
    const title =
      props.lang === "zh" ? "VS Code 通知" : props.lang === "zh-hant" ? "VS Code 通知" : "VS Code notification";
    return (
      <Shell
        open
        title={title}
          ...
          <div class="modal-actions action-popover-choice-grid">
            {request.actions.map((action) => (
              <button
                ...
                class="modal-primary-btn"
                type="button"
                onClick={() => props.onConfirm(request, [action])}
              >
                {action}
              </button>
            ))}
    ...
    return (
      ...
      >
        <div class="modal-stack">
          <div class="modal-note">{request.message}</div>
          <div class="modal-actions action-popover-choice-grid">
            <button class="modal-primary-btn danger" type="button" onClick={() => props.onConfirm(request, ["unload"])}>
              {request.unloadLabel}
            </button>
            <button class="modal-secondary-btn" type="button" onClick={() => props.onConfirm(request, ["keep"])}>
    ...
    <div class="action-popover-layer" aria-hidden={!props.open}>
      ...
      >
        <div class="action-popover-head">
          ...
          <button class="action-popover-close" type="button" aria-label={props.closeLabel} onClick={props.onClose}>
            ×
          </button>
        </div>
