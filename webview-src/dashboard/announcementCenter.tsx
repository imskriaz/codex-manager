...
export function AnnouncementCenter(props: {
  ...
  const listAccessibility = useModalAccessibility(props.open && detail === null, props.onClose);
  const detailAccessibility = useModalAccessibility(detail !== null, () => closeDetail(false));
  ...
  return (
    <>
      <div
        class={`overlay announcement-overlay ${props.open && !detail ? "open" : ""}`}
        aria-hidden={!props.open || detail !== null}
        onClick={props.onClose}
      >
        <div
          ref={listAccessibility.modalRef}
          class="settings-modal dashboard-modal announcement-list-modal"
          role="dialog"
          aria-modal="true"
          aria-label={props.copy.announcementsTitle}
          tabIndex={-1}
          onKeyDown={listAccessibility.onKeyDown}
      ...
      {detail ? (
        <div class="overlay announcement-overlay open" onClick={() => closeDetail(false)}>
          <div
            ref={detailAccessibility.modalRef}
            class="settings-modal dashboard-modal announcement-detail-modal"
            role="dialog"
            aria-modal="true"
            aria-label={detail.title}
            tabIndex={-1}
            onKeyDown={detailAccessibility.onKeyDown}
            onClick={(event) => event.stopPropagation()}
          >
